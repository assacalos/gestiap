...
class _DevisFormPageState extends State<DevisFormPage> {
  ...
  void initState() {
    ...
    Provider.of<ClientProvider>(
      context,
      listen: false,
    ).loadClients(); // Load clients if not already loaded
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isEditing ? "Modifier un devis" : "Créer un devis"),
      ),
        ...
        children: [
          FloatingActionButton.extended(
            ...
            onPressed: () {
              ...
              if (currentQuote != null) {
                quoteProvider.saveDraftQuote(currentQuote);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Devis enregistré comme brouillon.'),
                  ),
                );
              }
            },
          ...
          FloatingActionButton.extended(
            ...
            onPressed: () {
              final currentQuote = _getCurrentQuoteFromForm();
              if (currentQuote != null) {
                quoteProvider.updateQuote(
                  currentQuote,
                ); // Utilisez updateQuote pour la modification
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      isEditing
                          ? 'Devis mis à jour.'
                          : 'Devis soumis pour validation.',
                    ),
                  ),
                );
            ...
            label: const Text(
              "Sauvegarder",
            ), // Changement de label pour la modification
          ),
        ],
      ),
      body: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              if (widget.devisToEdit != null &&
                  widget.devisToEdit!.status == "En attente")
                Center(
                  child: ElevatedButton.icon(
                    icon: const Icon(Icons.check),
                    label: const Text("Valider le devis"),
                    onPressed: () async {
                      final updated = widget.devisToEdit!.copyWith(
                        status: "Validé",
                      );
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("Devis validé !")),
                      );
                      Navigator.pop(context);
                    },
                  ),
    ...
    return QuoteModel(
      ...
      status:
          isEditing && widget.devisToEdit != null
              ...
              : QuoteModel
                  .statusDraft, // Garder le statut existant ou mettre à brouillon par défaut

...
import 'package:gestiap/features/comptable/views/pages/charges/charge_form_page.dart';
...
class DepensesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<ChargesProvider>(
        builder: (context, depenseProvider, child) {
          if (depenseProvider.isLoading) {
            return const Center(child: CircularProgressIndicator());
          }
          if (depenseProvider.charges.isEmpty) {
            return const Center(child: Text('Aucune dépense enregistrée.'));
          }
          return ListView.builder(
            itemCount: depenseProvider.charges.length,
            itemBuilder: (context, index) {
              final depense = depenseProvider.charges[index];
              return Card(
                          ...
                          IconButton(
                            ...
                            onPressed: () {
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder:
                                      (context) => ChargesFormPage(
                                        chargeToEdit: depense,
                                      ),
                                ),
                              );
                            },
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => ChargesFormPage()),
          );
        },
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          actions: <Widget>[
            ...
            TextButton(
              onPressed: () {
                Provider.of<ChargesProvider>(
                  context,
                  listen: false,
                ).deleteCharge(depenseId);

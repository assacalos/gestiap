...
class FacturesPage extends StatelessWidget {
  ...
  const FacturesPage({super.key, this.statusFilter}); // Modify the constructor
    ...
    return Scaffold(
      ...
      body: Consumer<InvoiceProvider>(
        builder: (context, invoiceProvider, child) {
          ...
          final invoicesToDisplay =
              statusFilter != null
                  ? invoiceProvider.invoices
                      .where((invoice) => invoice.status == statusFilter)
                      .toList()
                  : invoiceProvider.invoices;

          if (invoicesToDisplay.isEmpty) {
            return const Center(child: Text('Aucune facture disponible.'));
          }
          return ListView.builder(
            itemCount: invoicesToDisplay.length,
            itemBuilder: (context, index) {
              final invoice = invoicesToDisplay[index];
              return Card(
